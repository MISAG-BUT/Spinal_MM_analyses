...
RESULTS_DIR = os.path.join(ROOT, "results", ANALYSIS_NAME)
os.makedirs(RESULTS_DIR, exist_ok=True)

OUT_ALL = os.path.join(RESULTS_DIR, "results_all_folds.csv")
OUT_MEAN = os.path.join(RESULTS_DIR, "results_model_means.csv")
OUT_IMPORTANCE = os.path.join(RESULTS_DIR, "feature_importance.csv")
    ...
    for model in os.listdir(base_dir):
        ...
        for fold in range(5):
            ...
            json_file = os.path.join(
                model_dir,
                f"{ANALYSIS_NAME}_fold_{fold}.json"
            )
...
# =====================================================
# STEP 4 — PLOT FEATURE IMPORTANCE PER DATASET
# =====================================================
...
if len(importance_df) > 0:
    ...
    for dataset in datasets:
        ...
        ].sort_values("Dice_drop", ascending=False)
        ...
        plt.title(f"Channel importance ({dataset})")
        ...
        plot_path = os.path.join(
            RESULTS_DIR,
            f"feature_importance_{dataset}.png"
        )
